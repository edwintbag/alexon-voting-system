// components/voting/StepSubmit.tsx — clean individual vote submit
"use client";
    ...
    <CardWrapper>
          ...
          {criteria.map(c => (
            <div key={c.key} className="flex items-center justify-between py-1">
              ...
              <div className="flex items-center gap-2">
                ...
                }`}>{SCORE_LABELS[ratings[c.key]] ?? "—"}</span>
      ...
      {error && (
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
          className="mt-4 p-3 bg-red-500/10 border border-red-500/30 rounded-lg text-red-400 text-sm">❌ {error}</motion.div>
      )}
      ...
      <div className="mt-8 flex justify-between">
        ...
        <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={handleSubmit} disabled={isSubmitting}
          ...
          {isSubmitting ? (<><svg className="animate-spin h-4 w-4" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/></svg>Submitting...</>) : "Submit Vote ✓"}
        </motion.button>
      </div>
    </CardWrapper>
  );
