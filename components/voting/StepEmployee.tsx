// components/voting/StepEmployee.tsx — v3 uses categoryId
"use client";
  ...
  return (
    <CardWrapper>
      <div className="mb-6">
        ...
        <p className="text-dark-400 mt-1 text-sm">Choose one person from <span className="text-gold-400">{category.name}</span> to nominate.</p>
      </div>

      {loading && <div className="space-y-3">{[1,2,3,4].map(i => <div key={i} className="skeleton h-16 rounded-xl" />)}</div>}
      {error && <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-lg text-red-400 text-sm">Failed to load candidates: {error}</div>}
        ...
        <div className="space-y-2 max-h-96 overflow-y-auto pr-1">
          {eligible.length === 0 ? (
            ...
            eligible.map((emp, i) => {
              ...
              return (
                <motion.button key={emp.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="mt-8 flex justify-between">
        ...
        <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={onNext}
          disabled={!selectedCandidate} className="btn-gold disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:scale-100">Continue →</motion.button>
      </div>
    </CardWrapper>
  );
}
