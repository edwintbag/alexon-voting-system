// components/admin/LeaderboardTable.tsx
"use client";
...
export default function LeaderboardTable({ results }: Props) {
  if (results.length === 0) {
    return (
    ...
    <div className="glass-card overflow-hidden">
        ...
        {results.map((result, i) => {
          ...
          return (
            <motion.div
              ...
              className={`grid grid-cols-12 gap-2 px-4 py-3.5 items-center relative overflow-hidden hover:bg-white/2 transition-colors ${
                result.rank === 1 ? "bg-gold-500/5" : ""
              }`}
            >
              {/* Rank */}
              <div className="col-span-1 flex items-center gap-1">
                <span
                  className={`text-sm ${RANK_STYLES[result.rank] ?? "text-dark-400"}`}
                >
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold ${
                      result.rank === 1
                        ...
                        : "bg-surface-border text-dark-300"
                    }`}
                  >
                    {result.employeeName
                      .split(" ")
                  ...
                  <span
                    className={`text-sm font-medium truncate ${
                      result.rank === 1 ? "text-gold-300" : "text-dark-100"
                    }`}
                  >
                    {result.employeeName}
                  </span>
              ...
              <div className="col-span-3 hidden sm:block">
                <span className="text-xs text-dark-400 truncate">
                  {DEPARTMENT_LABELS[result.department as keyof typeof DEPARTMENT_LABELS] ?? result.department}
                </span>
              </div>
                ...
                <div className="flex items-center justify-between mb-1">
                  <span
                    className={`text-sm font-mono font-bold ${
                      result.rank === 1 ? "text-gold-400" : "text-dark-200"
                    }`}
                  >
      ...
      <div className="px-4 py-3 border-t border-surface-border">
        <p className="text-xs text-dark-600">
          Final Score = (Avg Rating × 70%) + (Vote Count × 30% normalized)
        </p>
      </div>
    </div>
  );
