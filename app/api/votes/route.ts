...
// app/api/votes/route.ts — enforces voting window properly
import { NextRequest, NextResponse } from "next/server";
import { prisma } from "@/lib/prisma";
import { computeTotalScore, getVotingWindowStatus, getCurrentVotePeriod } from "@/lib/scoring";
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── VOTING WINDOW CHECK — enforced here ───────────
    ...
    // Validate candidate is in the category
    ...
    if (!candidateMember) {
      return NextResponse.json({ error: "Candidate is not a member of this category." }, { status: 400 });
    }
    ...
    return NextResponse.json(
      { success: true, voteId: vote.id, totalScore },
      { status: 201 }
    );
  } catch (error: any) {
