...
import { computeAvgRating, isVotingWindowOpenAsync, getCurrentVotePeriod } from "@/lib/scoring";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!windowOpen) {
      return NextResponse.json({ error: "Voting is not currently open. Opens on the 25th of each month." }, { status: 403 });
    }
    ...
    const vote = await prisma.$transaction(async (tx) => {
      const newVote = await tx.vote.create({
        data: { voterEmployeeId, candidateId, categoryId, voteMonth: month, voteYear: year, averageRating, comment: comment || null },
      });
      await tx.rating.createMany({
        data: Object.entries(ratings).map(([criterion, score]) => ({ voteId: newVote.id, criterion, score: score as number })),
      });
      return newVote;
    });

    return NextResponse.json({ success: true, voteId: vote.id, averageRating }, { status: 201 });
  } catch (error: any) {
